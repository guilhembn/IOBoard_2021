...
Arm::Arm(unsigned int pumpPin, unsigned int valvePin, unsigned int pressureSensorPin, HardwareSerial* dynamixelSerial, unsigned int zAxisStepPin, 
            unsigned int zAxisDirPin, unsigned int zAxisEnablePin, unsigned int zAxisLimitSwitchPin, unsigned int zRotDynamixelId, unsigned int yRotDynamixelId):
            dynamixelSerial_(dynamixelSerial), zAxisRotDynamixelId_(zRotDynamixelId), yAxisRotDynamixelId_(yRotDynamixelId),
            vacuumSystem_(pumpPin, valvePin, pressureSensorPin), zAxisStepper_(AccelStepper::DRIVER, zAxisStepPin, zAxisDirPin),
            zAxisLimitSwitchPin_(zAxisLimitSwitchPin), isStepperInSpeedMode_(false){
...
bool Arm::isMoving(){
    return zAxisStepper_.isRunning();
}
